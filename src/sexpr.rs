...
fn test_sexpr() {
    ...
    let expr = Expr::encode(&Expr::list(vec![
        Expr::symbol("module".to_string()),
        Expr::list(vec![
            Expr::symbol("global".to_string()),
            Expr::symbol("$hoge".to_string()),
            Expr::list(vec![
                Expr::symbol("mut".to_string()),
                Expr::symbol("i32".to_string()),
            ]),
            Expr::list(vec![Expr::symbol("i32.const".to_string()), Expr::int(1)]),
        ]),
    ]));

    assert_eq!(expr, "(module (global $hoge (mut i32) (i32.const 1)))");
}
